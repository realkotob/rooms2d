...
r.U=function(a){a.id=this.m;a.channel="widget";a=Nd(a);var b=this.i;var c=Ub(this.h.src||"");b=0==c.indexOf("https:")?[c]:b.h?[c.replace("http:","https:")]:b.j?[c]:[c,c.replace("http:","https:")];if(this.h.contentWindow)for(c=0;c<b.length;c++)try{this.h.contentWindow.postMessage(a,b[c])}catch(I){if(I.name&&"SyntaxError"==I.name){if(!(I.message&&0<I.message.indexOf("target origin ''"))){var d=void 0,e=I;d=void 0===d?{}:d;d.name=Q("INNERTUBE_CONTEXT_CLIENT_NAME",1);d.version=Q("INNERTUBE_CONTEXT_CLIENT_VERSION",
void 0);var f=d||{};d="WARNING";d=void 0===d?"ERROR":d;if(e){if(S("console_log_js_exceptions")){var g=e,h=[];h.push("Name: "+g.name);h.push("Message: "+g.message);g.hasOwnProperty("params")&&h.push("Error Params: "+JSON.stringify(g.params));h.push("File name: "+g.fileName);h.push("Stacktrace: "+g.stack);window.console.log(h.join("\n"),g)}if(!(5<=qh||0===e.sampleWeight)){var k=f,l=Jb(e);f=l.message||"Unknown Error";g=l.name||"UnknownError";var m=l.stack||e.h||"Not available";m.startsWith(g+": "+f)&&
(h=m.split("\n"),h.shift(),m=h.join("\n"));h=l.lineNumber||"Not available";l=l.fileName||"Not available";var n=void 0,p=e;if(p.hasOwnProperty("args")&&p.args&&p.args.length){var q=0;for(n=0;n<p.args.length;n++){var u=p.args[n],v="params."+n;q+=v.length;if(u)if(Array.isArray(u))for(var x=k,D=0;D<u.length&&!(u[D]&&(q+=nh(D,u[D],v,x),500<q));D++);else if("object"===typeof u)for(x in x=void 0,D=k,u){if(u[x]&&(q+=nh(x,u[x],v,D),500<q))break}else k[v]=oh(u),q+=k[v].length;else k[v]=oh(u),q+=k[v].length;
if(500<=q)break}}else if(p.hasOwnProperty("params")&&p.params)if(u=p.params,"object"===typeof p.params)for(n in p=0,u){if(u[n]&&(v="params."+n,q=oh(u[n]),k[v]=q,p+=v.length+q.length,500<p))break}else k.params=oh(u);navigator.vendor&&!k.hasOwnProperty("vendor")&&(k.vendor=navigator.vendor);f={message:f,name:g,lineNumber:h,fileName:l,stack:m,params:k};e=Number(e.columnNumber);isNaN(e)||(f.lineNumber=f.lineNumber+":"+e);e=f;f=w(jh);for(g=f.next();!g.done;g=f.next())if(g=g.value,g.T[e.name])for(l=w(g.T[e.name]),
h=l.next();!h.done;h=l.next())if(m=h.value,h=e.message.match(m.regexp)){e.params["params.error.original"]=h[0];l=m.groups;m={};for(k=0;k<l.length;k++)m[l[k]]=h[k+1],e.params["params.error."+l[k]]=h[k+1];e.message=g.Z(m);break}window.yterr&&"function"===typeof window.yterr&&window.yterr(e);f=S("kevlar_js_fixes")?!1:0<=e.stack.indexOf("/YouTubeCenter.js")||0<=e.stack.indexOf("/mytube.js");if(!ph.has(e.message)&&!f){"ERROR"===d?(mh.I("handleError",e),S("record_app_crashed_web")&&0===rh&&ih("appCrashed",
{appCrashType:"APP_CRASH_TYPE_BREAKPAD"}),rh++):"WARNING"===d&&mh.I("handleWarning",e);if(S("kevlar_gel_error_routing")){g=d;f=e;a:{h=w(sh);for(l=h.next();!l.done;l=h.next())if((m=J)&&0<=m.toLowerCase().indexOf(l.value.toLowerCase())){h=!0;break a}h=!1}if(!h){l={stackTrace:f.stack};f.fileName&&(l.filename=f.fileName);h=f.lineNumber&&f.lineNumber.split?f.lineNumber.split(":"):[];0!==h.length&&(1!==h.length||isNaN(Number(h[0]))?2!==h.length||isNaN(Number(h[0]))||isNaN(Number(h[1]))||(l.lineNumber=Number(h[0]),
l.columnNumber=Number(h[1])):l.lineNumber=Number(h[0]));lh||(lh=new kh);h=lh;m=f.message;k=f.name;a:{p=w(h.i);for(n=p.next();!n.done;n=p.next())if(n=n.value,f.message&&f.message.match(n.h)){p=n.weight;break a}p=w(h.h);for(n=p.next();!n.done;n=p.next())if(n=n.value,n.h(f)){p=n.weight;break a}p=1}m={level:"ERROR_LEVEL_UNKNOWN",message:m,errorClassName:k,sampleWeight:p};"ERROR"===g?m.level="ERROR_LEVEL_ERROR":"WARNING"===g&&(m.level="ERROR_LEVEL_WARNNING");g={isObfuscated:!0,browserStackInfo:l};l={pageUrl:window.location.href};
Q("FEXP_EXPERIMENTS")&&(l.experimentIds=Q("FEXP_EXPERIMENTS"));l.kvPairs=[{key:"client.params.errorServiceSignature",value:"msg="+h.i.length+"&cb="+h.h.length},{key:"client.params.serviceWorker",value:"false"}];if(f=f.params)for(h=w(Object.keys(f)),k=h.next();!k.done;k=h.next())k=k.value,l.kvPairs.push({key:"client."+k,value:String(f[k])});f=Q("SERVER_NAME",void 0);h=Q("SERVER_VERSION",void 0);f&&h&&(l.kvPairs.push({key:"server.name",value:f}),l.kvPairs.push({key:"server.version",value:h}));ih("clientError",
{errorMetadata:l,stackTrace:g,logMessage:m});xf()}}if(!S("suppress_error_204_logging")){g=e;f=g.params||{};d={ya:{a:"logerror",t:"jserror",type:g.name,msg:g.message.substr(0,250),line:g.lineNumber,level:d,"client.name":f.name},A:{url:Q("PAGE_NAME",window.location.href),file:g.fileName},method:"POST"};f.version&&(d["client.version"]=f.version);if(d.A){g.stack&&(d.A.stack=g.stack);g=w(Object.keys(f));for(h=g.next();!h.done;h=g.next())h=h.value,d.A["client."+h]=f[h];if(f=Q("LATEST_ECATCHER_SERVICE_TRACKING_PARAMS",
void 0))for(g=w(Object.keys(f)),h=g.next();!h.done;h=g.next())h=h.value,d.A[h]=f[h];f=Q("SERVER_NAME",void 0);g=Q("SERVER_VERSION",void 0);f&&g&&(d.A["server.name"]=f,d.A["server.version"]=g)}Sf(Q("ECATCHER_REPORT_HOST","")+"/error_204",d)}ph.add(e.message);qh++}}}}}else throw I;}else console&&console.warn&&console.warn("The YouTube player is not attached to the DOM. API calls should be made after the onReady event. See more: https://developers.google.com/youtube/iframe_api_reference#Events")};function zh(a){return(0===a.search("cue")||0===a.search("load"))&&"loadModule"!==a}
